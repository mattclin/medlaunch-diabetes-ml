...
# coding: utf-8
# ---------------------------------------------------------------
# Adapted from Hadia Hameed's script preprocess_oaps.py (https://github.com/health-ai-lab/BGLP_BG_forcasting)
# Date: 1/12/2022
# 
...
def move_csvfiles():
    for subj in subjs:
        ...
        fn = '%s/**/*.csv'%(subj_data_dir)
        if path.exists(subj_data_dir) == True:
            print('-------------------Moving files for subj '+subj+'-------------------')
            files = glob.glob(fn, recursive=True) #get all gz.json files in the directory
            for file in files:
                if os.path.isfile(file):
                    print(file, "exists")
                    filename = file.split('/')[-1] #get filename
                    ...
                    try:
                        ...
                        new_filename = extracted_folder_dir+filename
                        print('-------------------------')
                        ...
                        print("Moving csv files for subject: " + subj + "\n")
...
def get_subject_data(subj):
    ...
    if path.exists(subj_data_file) == True:
        files = glob.glob(subj_data_file+subj+'_'+feature+"*.csv")
        # print(files)
        files.sort()
        for f in files:
            df = pd.read_csv(f)
            ...
            if all(elem in df.columns for elem in attributes):
                ...
                if not df.empty:
                    if feature == 'entries':
                        df.drop_duplicates(subset=attributes[0:1], keep='first', inplace=True)
                        data_frames.append(df[attributes])
                        print ('Data frames updated', data_frames)
                    if feature == 'devicestatus' and 'openaps' in df.columns:
                        data_frames.append(df[attributes])
                    if feature == 'treatments':
                        data_frames.append(df[attributes])
                        print ('Treatment Data frames updated', data_frames)
            else:
                continue
...
def create_time_series():
    ...
    for subj in subjs:
        ...
        cgm_path = '%s%s/direct-sharing-31/extracted_files/complete_data_entries_%s.csv' %(data_directory,subj,subj)
        
        #cgm data or insulin data not present for subject, just continue
        if path.exists(insulin_path) == False: 
            print(insulin_path, ' path does not exist') 
            continue
        
        if path.exists(cgm_path) == False:
            print(cgm_path, ' path does not exist')
            continue
        # print ('Paths exist')
        ...
        original_df['glucose_level'] = pd.to_numeric(original_df['glucose_level'], errors='coerce')
